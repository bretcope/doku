...
function Doku(options)
{
	...
	function _formatFile(fileObj, markdown)
	{
	...
	function _generateDirectory (dirObj)
	{
		...
		if (dirObj.files.length > 0)
		{
			...
			for (i in dirObj.files)
			{
				...
				md = _generateFile(f, raw);
				html = _formatFile(f, md);
	...
	function _generateFile (fileObj, raw)
	{
		var i, c;
		...
		for (i = 0; i < comments.length; i++)
		{
			...
			c.includeCode = true;
			c.expandCode = null;
			c.isCode = c.type !== 'Line';
			c.linePrefix = _linePrefix(c, raw);
			
			_this.emit('comment', c);
		...
		for (i = 0; i < blocks.length; i++)
		{
			...
			_this.emit('code', b.code, b.comment);
			
			if (b.comment)
				mdBuilder.push(b.comment.value);
			
			if ((!b.comment || b.comment.includeCode) && b.code.value)
				mdBuilder.push(_codeToMarkdown(b.code.value));
		}
		
		//compile markdown
		var md = mdBuilder.join('');
		
		var page = { original: raw, markdown: md };
